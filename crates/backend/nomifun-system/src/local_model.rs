...
mod tests {
    ...
    fn catalog_is_small_curated_and_pinned() {
        ...
        for model in catalog {
            assert!(model.entry.download_size_bytes <= 3_000_000_000);
            assert!(model.entry.context_window <= 8192);
    ...
    #[tokio::test]
    #[ignore = "downloads about 520 MB and starts the real llama-server runtime"]
    async fn real_qwen_0_6b_install_and_streaming_smoke_test() {
        let temp = TempDir::new().unwrap();
        let db = init_database_memory().await.unwrap();
        let provider_repo: Arc<dyn IProviderRepository> =
            Arc::new(SqliteProviderRepository::new(db.pool().clone()));
        let (service, server) =
            start_and_provision_local_model(temp.path(), provider_repo, [7_u8; 32])
                .await
                .unwrap();
        let model_id = "qwen3-0.6b-q4-k-m";
    ...
    vec![
        ModelArtifact {
            entry: LocalModelCatalogEntry {
                ...
                name: "Qwen3 0.6B 极速版".into(),
                description: "适合分类、提取、改写和短摘要，资源占用最低。".into(),
                parameter_size: "0.6B".into(),
                quantization: "Q4_K_M".into(),
                download_size_bytes: 484_220_320,
                required_memory_bytes: 2 * 1024 * 1024 * 1024,
                context_window: 4096,
                license: "Apache-2.0".into(),
                source: "Qwen3 / bartowski GGUF".into(),
                recommended: false,
                tasks: vec![ModelTask::Chat],
                traits: vec![],
            },
            file_name: "qwen3-0.6b-q4-k-m.gguf",
        ...
        ModelArtifact {
            entry: LocalModelCatalogEntry {
                ...
                name: "Qwen3 1.7B 均衡版".into(),
                description: "兼顾中文对话质量和普通电脑上的运行速度，默认推荐。".into(),
                parameter_size: "1.7B".into(),
                quantization: "Q4_K_M".into(),
                download_size_bytes: 1_282_439_584,
                required_memory_bytes: 4 * 1024 * 1024 * 1024,
                context_window: 4096,
                license: "Apache-2.0".into(),
                source: "Qwen3 / bartowski GGUF".into(),
                recommended: true,
            ...
            file_name: "qwen3-1.7b-q4-k-m.gguf",
        ...
        ModelArtifact {
            entry: LocalModelCatalogEntry {
                ...
                name: "Qwen3 4B 质量版".into(),
                description: "面向内存较充足的设备，提供更好的本地文本对话质量。".into(),
                parameter_size: "4B".into(),
                quantization: "Q4_K_M".into(),
                download_size_bytes: 2_497_280_960,
                required_memory_bytes: 7 * 1024 * 1024 * 1024,
                context_window: 8192,
                license: "Apache-2.0".into(),
                source: "Qwen3 / bartowski GGUF".into(),
                recommended: false,
            ...
            file_name: "qwen3-4b-q4-k-m.gguf",
            url: "https://huggingface.co/bartowski/Qwen_Qwen3-4B-GGUF/resolve/cb76885dc66d50759b207c5a48c4e78dfa00c638/Qwen_Qwen3-4B-Q4_K_M.gguf",
